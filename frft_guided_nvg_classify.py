...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GNN classifier for NVG graphs generated by nvg_filter_then_compare_merged_v2.py"
    )
...
if __name__ == "__main__":
    ...
    results_txt = os.path.join(
        BASE_OUT,
        f"results_{dataset_tag}_{TRANSFORM}_{FILTER_KIND}.txt"
    )
    ...
    with open(results_txt, "a", buffering=1, encoding="utf-8") as log_f:
        ...
        print(f"TRANSFORM={TRANSFORM} | FILTER_KIND={FILTER_KIND}")
        ...
        for order in ORDERS:
            ...
            for SEED in SEEDS:
                ...
                # Fresh split per seed: ~70% train, 15% val, 15% test
                ...
                tag = f"{TRANSFORM}_order{order}_{FILTER_KIND}"
